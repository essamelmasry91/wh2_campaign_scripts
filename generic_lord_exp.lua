...
if not cm:get_saved_value("player_leader_rank") then cm:set_saved_value("player_leader_rank", 10); end;

core:add_listener (
	"generic_CharactersGetEXP",
	"CharacterTurnStart",
	function(context)
...
function(context)
	...
	local char_name = context:character():get_forename()
	local player_rank = cm:get_saved_value("player_leader_rank");
	if rank <= player_rank then
		if not legendary_lords_subtype[char:character_subtype_key()] then
			if rank <= 10 then
			cm:add_agent_experience(char_str, 1500, false)
			elseif rank <= 20 and rank > 10 then
			cm:add_agent_experience(char_str, 1200, false)
			elseif rank <= 30 and rank > 20 then 
			cm:add_agent_experience(char_str, 500, false)
			end
		end
	end;
end,
...
core:add_listener(
	"faction_turn_start_getplayerrank",
	"FactionTurnStart",
	function(context) return context:faction():is_human() == true; end,
	function()
local player_faction_leader = cm:model():world():faction_by_key(cm:model():world():whose_turn_is_it():name()):faction_leader()
local player_faction_leader_cqi = cm:model():world():faction_by_key(cm:model():world():whose_turn_is_it():name()):faction_leader():command_queue_index()
local player_faction_leader_str = cm:char_lookup_str(player_faction_leader_cqi)
local player_rank =  player_faction_leader:rank()
cm:set_saved_value("player_leader_rank", player_rank)
end,
true
);